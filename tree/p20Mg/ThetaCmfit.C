{
TCanvas canvas("ThetaCM");

  //For fitting the 19Na -> p + 18Ne Vcm
  TFile f("sort.root");
  TH1I * ThetaCM_B = (TH1I*) f.Get("SortCode/ThetaCM_21Al_p20Mg");

  //Array of parameters
  double par[6];

  TF1 *g1 = new TF1("g1", "gaus", 0, 1);
  TF1 *g2 = new TF1("g2", "gaus", 1,2.3);


  TF1 *total = new TF1("total", "gaus(0)+gaus(3)", 0, 2.3);

   ThetaCM_B->Draw();
   ThetaCM_B->Fit(g1, "0R+");
   ThetaCM_B->Fit(g2, "0R+");

   g1->GetParameters(&par[0]);
   g2->GetParameters(&par[3]);

   total->SetParameters(par);
   ThetaCM_B->Fit(total, "0R+");

   total->GetParameters(&par[0]);

   g1->SetParameters(&par[0]);
   g2->SetParameters(&par[3]);

   total->Draw("same");
   g1->Draw("same");
   g2->Draw("same");

   double inttot = (par[0]*fabs(par[2]) + par[3]*fabs(par[5]))*sqrt(2*M_PI);
   cout << "Total Integral " << inttot << endl;
   cout << "amp ratio 1 " << par[0]*fabs(par[2])*sqrt(2*M_PI)/inttot << endl;
   cout << "amp ratio 2 " << par[3]*fabs(par[5])*sqrt(2*M_PI)/inttot << endl;

}
